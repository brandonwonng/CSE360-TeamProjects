...
public class GUIDeleteUserPage {
	...
	**********************************************************************************************/
	...
	private Button button_deleteUser = new Button("Delete this user");
	private List<String> removeList;

	
	private Line line_Separator1 = new Line(20, 95, FCMainClass.WINDOW_WIDTH-20, 95);

	private Label label_SelectUser = new Label("Select a user to be deleted:");
...
//	private String [] roles = {"Admin", "Student", "Reviewer", "Instructor", "Staff"};
	...
	**********************************************************************************************/
	...
	private void setupSelectedUser() {
		...
		setupButtonUI(button_deleteUser, "Dialog", 16, 150, Pos.BASELINE_LEFT, 535, 125);
		button_deleteUser.setOnAction((event) -> {performDeleteUser(); });
	...
	private void performDeleteUser() {
		if (theSelectedUser == null || theSelectedUser.equals("<Select a User>")) {
		...
		if (theSelectedUser.equals(theUser.getUserName())) {
			...
			errorAlert.setHeaderText("Action Not Allowed");
			errorAlert.setContentText("You cannot delete yourself.");
		...
		if (result.isPresent() && result.get() == yesButton) {
		    ...
		    if (theDatabase.deleteUser(theSelectedUser)) {
		        System.out.println("User deleted successfully: " + theSelectedUser);

		        // Refresh the user list in the ComboBox
		        List<String> updatedUserList = theDatabase.getUserList();
		        combobox_SelectUser.setItems(FXCollections.observableArrayList(updatedUserList));
		        combobox_SelectUser.getSelectionModel().select(0);
		        ...
		        System.out.println("Failed to delete user: " + theSelectedUser);
		        Alert failAlert = new Alert(Alert.AlertType.ERROR);
		        failAlert.setTitle("Deletion Failed");
		        failAlert.setHeaderText("Unable to delete user.");
		        failAlert.setContentText("Please try again or contact support.");
